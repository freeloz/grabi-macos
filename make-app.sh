...
# Firma, perfil y entitlements: scripts/sign-app.sh, compartido con el
# script de release para que no vuelvan a divergir.
scripts/sign-app.sh "$APP" "$BUNDLE_ID"
[[ -n "$(security find-identity -v -p codesigning 2>/dev/null | grep "Developer ID Application")" ]] \
